...
type P = { id: number; name: string; oven_types: string }; type O = { id: number; label: string; oven_type: string };
...
export default function BatchesPage() {
  ...
  return (<>
    ...
    <div className="toolbar">
      <select value={pid} onChange={e => setPid(Number(e.target.value))}>{products.map(p => <option key={p.id} value={p.id}>{p.name}（{p.oven_types}）</option>)}</select>
      <select value={oid} onChange={e => setOid(Number(e.target.value))}>{ovens.map(o => <option key={o.id} value={o.id}>{o.label}（{o.oven_type}）</option>)}</select>
